...
export class MysqlConversationStreamStore implements ConversationStreamStore {
	...
	async getMeta(path: string): Promise<ConversationStreamMeta | null> {
		const rows = await this.runner.query('SELECT identity_json, next_offset, closed, producer_id, producer_epoch, next_producer_sequence FROM flue_conversation_streams WHERE path = ?', [path]);
		const row = rows[0];
		if (!row) return null;
		return {
...
export class MysqlConversationSnapshotStore<State = unknown> implements ConversationSnapshotStore<State> {
	...
	async save(path: string, snapshot: ConversationSnapshot<State>): Promise<void> {
		await this.runner.transaction(async (tx) => {
			const rows = await tx.query('SELECT next_offset FROM flue_conversation_streams WHERE path = ? FOR UPDATE', [path]);
			const meta = rows[0];
