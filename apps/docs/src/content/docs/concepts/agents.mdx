...
Out of the box, Flue provides a lightweight virtual sandbox — an in-memory filesystem and shell — so an agent has a usable environment with no setup. When you need more, you connect a different one behind the same API: direct access to the host machine, a platform-native option such as Cloudflare, or one of the popular sandbox providers. The [ecosystem](/docs/ecosystem/overview/) lists the available connectors.
