...
- Added first-party `@flue/github`, `@flue/slack`, `@flue/discord`, `@flue/teams`, and `@flue/google-chat` packages for verified HTTP ingress, constructor-owned typed handlers, canonical conversation identities, and discovered `channels/<name>.ts` routing. Named `flue add` recipes create editable project code using provider SDK or Fetch clients and application-owned tools.
